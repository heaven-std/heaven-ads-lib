...
class LanguageAdapter(
    ...
    inner class LanguageViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(language: AppLanguage) {
            vItem.setBackgroundResource(configLanguage.bgItemLanguage)
            tvLabel.text = language.name
            imgLanguage.setImageResource(language.flag)

            if (language.code == languageCode) {
                imgSelect.setImageResource(configLanguage.icSelectedLanguage)
            } else {
                imgSelect.setImageResource(configLanguage.icUnSelectedLanguage)
            }
